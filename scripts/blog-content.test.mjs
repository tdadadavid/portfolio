...
test('lists parts in reverse order while preserving the reading sequence', async t => {
    ...
    assert.equal(helpers.getAllBlogs().map(post => post.slug).join(','), 'post/log/1-part,post/log/2-part,post/log/3-part');
    ...
    assert.equal(helpers.getReadableBlogs()[0].slug, 'post/log/2-part');
    ...
    assert.equal(metadata.alternates.canonical, '/blog/post/log/3-part');
